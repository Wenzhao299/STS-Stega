...
class Resizer(nn.Module):
    def __init__(self, in_shape, scale_factor=None, output_shape=None, kernel=None, antialiasing=True):
        ...
        # N =32时中down[1, 1, 0.03125, 0.03125]  [1 3 8 8]
        # N =up[1, 1, 32, 32]  [  1   3 256 256]
        ...
        # bool值，down时是True up是false
        antialiasing *= (np.any(np.array(scale_factor) < 1))

        # Sort indices of dimensions according to scale of each dimension. since we are going dim by dim this is efficient
        # sorted_dims = [2 3 0 1]
        # np.argsort将x中的元素从小到大排列，提取其对应的index(索引)，然后输出到y
        sorted_dims = np.argsort(np.array(scale_factor))
        # self.sorted_dims[2, 3] 把 scale_factor中的1丢掉
        ...
        # print("self.field_of_view",self.field_of_view[0])
    ...
    def forward(self, in_tensor):
        ...
        # 1 3 256 256
        # print(x.shape)
        # print("self.weights", self.weights[1])
    ...
    def reverse(self,in_tensor,f,weights):
        x = in_tensor
        print(x.shape)

        self.sorted_dims = [3,2]
        for dim, fov, w in zip(self.sorted_dims, f, weights):
            x = torch.transpose(x, dim, 0)
            print(x.shape)
            x = torch.sum(x[fov] / w, dim=0)
            # x = torch.sum(x[fov] * w, dim=0)
