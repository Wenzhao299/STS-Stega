...
A batch processing script for steganography using STC in convolutional mode.
It uses the producer-consumer model to generate a dataset of cover/stega pairs,
where the steganography is performed by `stega_stc.py`.
"""
...
_STC_BINDINGS: Optional[Dict[str, Any]] = None


def _load_stc_bindings() -> Dict[str, Any]:
    global _STC_BINDINGS
    if _STC_BINDINGS is None:
        from stega_stc import (
            create_argparser,
...
def _create_batch_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Batch embed/extract runner for the hybrid STC method."
    )
    ...
    parser.add_argument("--stc-constraint-height", type=int, default=7)
    parser.add_argument("--stc-block-len", type=int, default=65536)
    parser.description = "Batch embed/extract runner for the hybrid STC method."
    parser.add_argument(
        "--num-images",
        type=int,
        default=100,
...
def _extract_stc_message(stego_path: str, metadata_path: str) -> str:
    ...
    seed = metadata["seed"]
    block_len = metadata["stc_block_len"]
    ...
    samplers_cache: Dict[Tuple[int, int, int, int], STCSampler] = {}
    extracted_stc_bits: list[int] = []
    ...
    for plane_idx in filling_order:
        ...
        for block_idx, payload_size in enumerate(payloads_per_block):
            ...
            extracted_stc_bits.extend(samplers_cache[sampler_key].matvec(block_bits).tolist())

    full_extracted_bits = np.concatenate(
        [
            np.asarray(extracted_lsb_bits, dtype=int),
            np.asarray(extracted_stc_bits[:stc_payload_len], dtype=int),
        ]
    )
...
def stc_producer(task_queue: multiprocessing.Queue, seed_list: List[int], gpu_id: int, base_args, temp_dir: str):
    """
...
def stc_consumer(task_queue: multiprocessing.Queue, done_queue: multiprocessing.Queue, base_save_dir: str, base_args, consumer_id: int):
    """
    Consumer: Uses CPU to perform the STC-based hybrid embedding logic.
    """
    # --- Suppress all output from this worker ---
    sys.stdout = open(os.devnull, 'w')
    ...
    while True:
        ...
        try:
            ...
            # We only need to generate the STC part and a placeholder for the LSB part.
            lsb_payload_len = args.image_size * args.image_size * 3
            if args.stc_payload:
                stc_payload_len = sum(args.stc_payload) * 3
            else:
                stc_payload_len = 0
            
            payload_size = lsb_payload_len + stc_payload_len
...
def run_stc_batch_experiment(
    ...
    gpus: List[int] = None,
    resume_dir: str = None,
    stc_payload: List[int] = None,
    base_args: Optional[argparse.Namespace] = None,
    output_root: str = "output_stc_parallel",
):
    """
    Main runner for the STC-based dataset generation experiment.
    """
    base_save_dir = ""
    temp_dir = ""
    base_args = _resolve_base_args(base_args)
    if stc_payload is not None:
        base_args.stc_payload = stc_payload
    try:
        ...
        if resume_dir:
            print(f"--- Resuming STC dataset generation in: {base_save_dir} ---")
        else:
            os.makedirs(base_save_dir, exist_ok=True)
            if using_existing_dir:
                print(f"--- Reusing STC dataset generation directory: {base_save_dir} ---")
            else:
                print(f"--- Running STC dataset generation in: {base_save_dir} ---")
        ...
        for gpu_id in gpu_ids:
            for _ in range(tasks_per_gpu):
                if producer_id_counter < len(tasks_for_producers):
                    ...
                    if len(seed_list) > 0:
                        p = ctx.Process(target=stc_producer, args=(task_queue, list(seed_list), gpu_id, base_args, temp_dir))
                        producers.append(p)
                        p.start()
        ...
        for i in range(num_consumers):
            consumer_pool.apply_async(stc_consumer, args=(task_queue, done_queue, base_save_dir, base_args, i))

        pbar = tqdm(total=len(seeds_to_generate), desc="STC Dataset Generation")
        completed_count = 0
        ...
        print("\n--- STC Dataset Generation Finished ---")
...
def run_stc_embedding(costs_batch, message_batch, h, verbose=False):
    """
    Run STC embedding on a batch of costs and messages.
    """
    stego_batch = []
    
    for costs, message in zip(costs_batch, message_batch):
        n = costs.shape[0]
        c = message.shape[0]
        
        # Instantiate the sampler with convolutional method
        sampler = STCSampler(c=c, h=h, n=n, H_construction_method='convolutional')
        
        # Convert costs to marginal probabilities
        exp_neg_costs = np.exp(-costs)
        sum_exp_neg_costs = np.sum(exp_neg_costs, axis=1, keepdims=True)
        marginal_probs = exp_neg_costs / sum_exp_neg_costs
        
        stego_bits, _ = sampler.sample_bit_plane(
        ...
        if stego_bits is None:
            raise RuntimeError("STC sampling failed and returned None.")
            
        stego_batch.append(stego_bits)
        
    return stego_batch


def run_stc_extraction(
    resume_dir: Optional[str] = None,
    stc_payload: Optional[List[int]] = None,
    base_args: Optional[argparse.Namespace] = None,
    output_root: str = "output_stc_parallel",
) -> Dict[str, Any]:
    ...
    for task_id in tqdm(task_ids, desc="STC Extraction"):
        ...
        try:
            extracted_message = _extract_stc_message(stego_path, metadata_path)
